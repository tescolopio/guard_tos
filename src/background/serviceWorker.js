...
import { EXT_CONSTANTS } from '../utils/constants.js';
    ...
    */
    async function analyzeContent(text) {
      try {
        ...
        const readability = results[0].status === 'fulfilled' ? results[0].value : { error: "Readability analysis failed" };
        const rights = results[1].status === 'fulfilled' ? results[1].value : { error: "Rights analysis failed" };
        const summary = results[2].status === 'fulfilled' ? results[2].value : { error: "Summarization failed" };
        const uncommonWords = results[3].status === 'fulfilled' ? results[3].value : { error: "Uncommon words identification failed" };
