...
describe('EXT_CONSTANTS', () => {
    ...
    test('should have correct analysis settings', () => {
        expect(EXT_CONSTANTS.ANALYSIS.PERFORMANCE_THRESHOLDS.TEXT_PROCESSING).toBe(100);
        expect(EXT_CONSTANTS.ANALYSIS.PERFORMANCE_THRESHOLDS.API_CALL).toBe(2000);
        expect(EXT_CONSTANTS.ANALYSIS.PERFORMANCE_THRESHOLDS.GRADE_CALCULATION).toBe(50);
        expect(EXT_CONSTANTS.ANALYSIS.PERFORMANCE_THRESHOLDS.RIGHTS_ANALYSIS).toBe(150);
    ...
    test('should have correct notification messages', () => {
        expect(EXT_CONSTANTS.MESSAGES.AUTO_GRADE).toBe("Terms Guardian has detected a legal document and is currently grading it. Click the extension badge at the top of the browser to see the readability and how it affects your rights by agreeing to it. This is for educational purposes only and is not legal advice.");
        expect(EXT_CONSTANTS.MESSAGES.SIGNIFICANT_TERMS).toBe("A significant number of legal terms have been found on this page. Click the Terms Guardian Extension badge at the top of the screen to grade the text. If this is not a legal document like a Terms of Service you can still grade sections of text by selecting the text you want to grade and right clicking to bring up the context menu, then click 'grade this text' to learn more about it. This is for educational purposes only and is not legal advice.");
        expect(EXT_CONSTANTS.MESSAGES.NO_LEGAL_TEXT).toBe("No legal text was found on this page.");
        expect(EXT_CONSTANTS.MESSAGES.ERROR.MODEL_LOAD).toBe("Error loading analysis model");
        expect(EXT_CONSTANTS.MESSAGES.ERROR.API_ERROR).toBe("Error communicating with definition service");
        expect(EXT_CONSTANTS.MESSAGES.ERROR.INVALID_TEXT).toBe("Invalid or empty text provided");
        expect(EXT_CONSTANTS.MESSAGES.ERROR.GENERAL).toBe("An unexpected error occurred. Please try again later.");
        expect(EXT_CONSTANTS.MESSAGES.ERROR.PERFORMANCE).toBe("Operation took longer than expected");
        expect(EXT_CONSTANTS.MESSAGES.ERROR.STORAGE_FULL).toBe("Storage limit reached");
        expect(EXT_CONSTANTS.MESSAGES.ERROR.NETWORK).toBe("Network connection error");
    });
    ...
    test('should have correct error types', () => {
        ...
        expect(EXT_CONSTANTS.ERROR_TYPES.EXTRACTION.INCOMPLETE_HTML).toBe('INCOMPLETE_HTML');
        expect(EXT_CONSTANTS.ERROR_TYPES.EXTRACTION.MALFORMED_HTML).toBe('MALFORMED_HTML');
        expect(EXT_CONSTANTS.ERROR_TYPES.EXTRACTION.PDF_EXTRACTION).toBe('PDF_EXTRACTION');
        expect(EXT_CONSTANTS.ERROR_TYPES.EXTRACTION.DOCX_EXTRACTION).toBe('DOCX_EXTRACTION');
        expect(EXT_CONSTANTS.ERROR_TYPES.EXTRACTION.INVALID_INPUT).toBe('INVALID_INPUT');
    });

    test('should have correct API settings', () => {
        expect(EXT_CONSTANTS.API.LEXPREDICT.BASE_URL).toBe('https://api.lexpredict.com/v1');
        expect(EXT_CONSTANTS.API.LEXPREDICT.ENDPOINTS.DEFINITIONS).toBe('/dictionary/legal/common-law');
        expect(EXT_CONSTANTS.API.LEXPREDICT.TIMEOUT).toBe(5000);
        expect(EXT_CONSTANTS.API.LEXPREDICT.RETRY_DELAYS).toEqual([1000, 2000, 4000]);
    });
    ...
    test('should have correct DOM selectors', () => {
        ...
        expect(EXT_CONSTANTS.SELECTORS.POPUPS.READABILITY).toBe('#readabilityPopup');
    ...
    test('should have correct context menu items', () => {
        ...
        expect(EXT_CONSTANTS.CONTEXT_MENU.GRADE_TEXT.contexts).toEqual(['selection']);
    });
});
