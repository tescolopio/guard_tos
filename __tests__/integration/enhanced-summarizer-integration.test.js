...
function testEnhancedSummarizer() {
  try {
    ...
    console.log(
      "- Enhanced features:",
      result.metadata ? result.metadata.enhancedSummary : false,
    );
    ...
    if (result.sections && result.sections.length > 0) {
      ...
      console.log(
        ...
        result.sections[0].userFriendlyHeading,
      );
    }
...
// Minimal Jest test to satisfy runner and smoke the helper
test("enhanced summarizer integration smoke test", () => {
  const ok = testEnhancedSummarizer();
  expect(ok).toBe(true);
});
